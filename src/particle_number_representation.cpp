...
#include "particle_number_representation.hpp"
#include "exatn.hpp"
#include "talshxx.hpp"
...
void ParticleNumberRepresentation::clear()
{
...
void ParticleNumberRepresentation::resetWaveFunctionAnsatz(std::shared_ptr<exatn::TensorNetwork> ansatz)
{
  clear();
  bra_ansatz_.reset();
  ket_ansatz_.reset();
...
void ParticleNumberRepresentation::resetWaveFunctionAnsatz(std::shared_ptr<exatn::TensorExpansion> ansatz)
{
  clear();
  bra_ansatz_.reset();
  ket_ansatz_.reset();
...
void ParticleNumberRepresentation::resetWaveFunctionAnsatz(exatn::NetworkBuilder & ansatz_builder)
{
 clear();
 //`Implement
 return;
...
void ParticleNumberRepresentation::resetHamiltonian(const std::vector<std::shared_ptr<exatn::Tensor>> & hamiltonian)
{
 clear();
...
void ParticleNumberRepresentation::markOptimizableTensors(){
  for (auto component = ket_ansatz_->begin(); component != ket_ansatz_->end(); ++component){
    component->network->markOptimizableAllTensors();
  }
}

bool ParticleNumberRepresentation::optimize(std::size_t num_states, double convergence_thresh){
...
void ParticleNumberRepresentation::appendOrderingProjectors(){
...
void ParticleNumberRepresentation::constructEnergyFunctional(){
}

void ParticleNumberRepresentation::constructEnergyDerivatives(){
}

void ParticleNumberRepresentation::initWavefunctionAnsatz(){
  const auto TENS_ELEM_TYPE = exatn::TensorElementType::REAL64;
...
double ParticleNumberRepresentation::evaluateEnergyFunctional(){
  double energy = 0.0;
  return energy;
}

void ParticleNumberRepresentation::evaluateEnergyDerivatives(){
}

void ParticleNumberRepresentation::updateWavefunctionAnsatzTensors(){
}
